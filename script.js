...
function canAccessPage(pageName) {
    ...
    const pagePermissions = {
        ...
        'raporlar': ['reports'],
        'admin': ['admin'],  // SADECE admin yetkisi olanlar
        'kullaniciAktiviteleri': ['admin'],  // SADECE admin yetkisi olanlar
        'kullaniciRaporlari': ['reports'],  // Raporlar yetkisi olanlar görebilir
        'insanKaynaklari': ['hr'],
        'dashboard': true,
        'profil': true,
        'ayarlar': true,
        ...
        if (user && user.password === password && user.active) {
            ...
            if (!user.permissions || user.permissions.length === 0) {
                const defaultPermissions = {
                    'admin': ['admin', 'sales', 'production', 'warehouse', 'logistics', 'reports', 'hr'],
                    'production': ['production', 'reports'],
                    'sales': ['sales', 'reports'],
                    'warehouse': ['warehouse', 'reports'],
                    'logistics': ['logistics', 'reports'],
                    'quality': ['production', 'reports']
                };
...
function loadIsEmriVer() {
    ...
    document.getElementById('pageContent').innerHTML = content;
    if (isMobileDevice()) {
        document.querySelectorAll('#recipePreview, #stockStatus').forEach(el => {
            el.style.display = 'block';
        });
    }
    applyMobileOptimizations('isEmriVer');
}
...
function closeModal(modalId) {
    ...
    if (modal) {
        modal.classList.remove('show');
        // Modal'ı tamamen kaldır (dinamik oluşturulanlar için)
        if (modalId === 'workDetailsModal' || modalId === 'rejectLeaveModal' || modalId === 'rawMaterialSelectModal' || modalId === 'recipeRawMaterialSelectModal') {
            setTimeout(() => {
                modal.remove();
            }, 300);
        }
...
function updateSidebarPermissions() {
    ...
    if (hasPermission('warehouse') || hasPermission('logistics') || hasPermission('reports')) {
        document.getElementById('operationSection').style.display = 'block';
        
        // Operasyon içindeki menüleri kontrol et
        if (!hasPermission('warehouse')) {
            document.getElementById('depoMenu').style.display = 'none';
        }
        if (!hasPermission('warehouse') && !hasPermission('logistics')) {
            document.getElementById('sevkiyatMenu').style.display = 'none';
        }
        if (!hasPermission('reports')) {
            document.getElementById('raporlarMenu').style.display = 'none';
        }
    }
